...
这是为你准备的 **「复制粘贴」级别** 指令手册。按照以下 1 到 8 步顺序依次执行，由于涉及到不同的技术栈，每一步都推荐了当前阶段最适合的 AI 模型。
...
## 第六步：GitHub 仓库初始化与云端测试 (GitHub Actions CI/CD)
**目标**：将项目推送到 GitHub，建立自动化测试流水线，实现云端验证。
* **推荐模型**：**Claude 3.7 Sonnet** 或 **GitHub Copilot** (对 GitHub Actions 和 CI/CD 流程最熟悉)
* **💡 发给 Agent 的提示词**：
> \"你是 Synch 项目的 DevOps 工程师。我们需要将项目推送到 GitHub 并建立 CI/CD 流水线。
> 1. 请在项目根目录初始化 Git 仓库（如未初始化），创建 `.gitignore` 排除 `target/`, `node_modules/`, `.gradle/` 等构建产物。
> 2. 创建 `README.md` 补充项目简介、架构图、快速开始指南。
> 3. 在 `.github/workflows/` 目录下创建 `ci.yml`，实现以下 Job：
>    - `rust-build`: 在 ubuntu-latest 上编译 Rust core，运行 `cargo test`
>    - `go-build`: 在 ubuntu-latest 上编译 Go relay 服务端
>    - `ts-build`: 在 ubuntu-latest 上构建 vcp-agent 插件
>    - `android-build`: 在 ubuntu-latest 上编译 Android 项目（使用 `actions/setup-java` 和 `actions/setup-android-sdk`）
>    - `integration-test`: 启动 relay 服务端，运行端到端测试验证 WebSocket 连接
> 4. 创建 GitHub 仓库（或使用现有仓库），推送代码，验证 Actions 运行成功。
> 5. 如有报错，请自动修复并重试，直到所有 Job 绿通。\"
...
在根目录创建 .gitignore，排除 target/、node_modules/、*.so、*.dylib、*.dll、build/、.env 等构建产物。
初始化 git 仓库，创建初始 commit：'init: Synch project bootstrap'。
创建 GitHub 仓库（假设用户已提供 token 或通过 gh CLI），推送到 origin main。
建立分支保护规则：main 分支需 PR 审查，develop 分支作为集成分支。
创建标签规范：v0.1.0-alpha 格式，为当前状态打初始标签。
...
创建 docker-compose.test.yml - 测试专用编排：
包含 server 服务、测试 runner、网络隔离
健康检查确保服务就绪后才执行测试
在 CI 中新增 test:e2e job，使用 GitHub Actions 的 service containers
生成覆盖率报告，上传 codecov
